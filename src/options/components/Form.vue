<template>
  <div>
    ...
    <div class="row  mt-1">
      <div class="col">
        <button type="button" class="btn  btn-primary" :disabled="disabled" @click="onSubmit">Add New Entry</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    domainValid() {
      return this.form.domain.length && this.alphaNumericRegex.test(this.form.domain);
    },
    colorValid() {
      return this.form.color.length && this.hexColorRegex.test(this.form.color);
    },
    disabled() {
